...
for(i=0;i<fonction;i++)
{
   ...
   /* j=0;
    fprintf(fichier,"%s, in:",modules[i].func);
    for(j=0;j<modules[i].n_v_in;j++)
    {
    fprintf(fichier,"%s,",modules[i].v_in[j].v);
    }
    fputs("\n",fichier);*/
    j=0;
    fprintf(fichier,"%s:",modules[i].func);
...
fclose(fichier);
    ...
    printf("Impossible d'ouvrir le fichier in_out.txt");
fclose(fichier2);
}
else
